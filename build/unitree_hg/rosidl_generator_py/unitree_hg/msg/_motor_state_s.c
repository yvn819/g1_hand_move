...
bool unitree_hg__msg__motor_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("unitree_hg.msg._motor_state.MotorState", full_classname_dest, 38) == 0);
  }
  unitree_hg__msg__MotorState * ros_message = _ros_message;
  {  // mode
  ...
  {  // vol
    PyObject * field = PyObject_GetAttrString(_pymsg, "vol");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->vol = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // sensor
    ...
    }
    {
      ...
      Py_ssize_t size = 2;
      uint32_t * dest = ros_message->sensor;
      for (Py_ssize_t i = 0; i < size; ++i) {
  ...
  {  // motorstate
    ...
    assert(PyLong_Check(field));
    ros_message->motorstate = PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // reserve
...
PyObject * unitree_hg__msg__motor_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("unitree_hg.msg._motor_state");
  ...
  {  // vol
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->vol);
    {
      int rc = PyObject_SetAttrString(_pymessage, "vol", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
  ...
  {  // sensor
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "sensor");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    uint32_t * src = &(ros_message->sensor[0]);
    memcpy(dst, src, 2 * sizeof(uint32_t));
    Py_DECREF(field);
  }
  {  // motorstate
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->motorstate);
    {
