...
bool unitree_hg__msg__press_sensor_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("unitree_hg.msg._press_sensor_state.PressSensorState", full_classname_dest, 51) == 0);
  }
  unitree_hg__msg__PressSensorState * ros_message = _ros_message;
  {  // pressure
    ...
    }
    {
      ...
      Py_ssize_t size = 12;
      float * dest = ros_message->pressure;
  ...
  {  // temperature
    ...
    }
    {
      ...
      assert(PyArray_TYPE(seq_field) == NPY_FLOAT32);
      Py_ssize_t size = 12;
      float * dest = ros_message->temperature;
      for (Py_ssize_t i = 0; i < size; ++i) {
        float tmp = *(npy_float32 *)PyArray_GETPTR1(seq_field, i);
        memcpy(&dest[i], &tmp, sizeof(float));
      }
...
{
  /* NOTE(esteve): Call constructor of PressSensorState */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("unitree_hg.msg._press_sensor_state");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "PressSensorState");
  ...
  {  // pressure
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "pressure");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    float * src = &(ros_message->pressure[0]);
  ...
  {  // temperature
    ...
    assert(sizeof(npy_float32) == sizeof(float));
    npy_float32 * dst = (npy_float32 *)PyArray_GETPTR1(seq_field, 0);
    float * src = &(ros_message->temperature[0]);
    memcpy(dst, src, 12 * sizeof(float));
    Py_DECREF(field);
  }
  {  // lost
